...
**ReAct orchestrator + Streamlit UI** — Natural language interface wrapping the full pipeline as LangChain tools (`attribute_domain`, `process_domains`, `search_cti`). Streamlit chat UI with confidence panel and metadata display.
...
│   │   ├── orchestrator.py  #     ReAct agent + tool definitions
│   │   ├── nodes/
...
# pip install -e ".[agent]"       # core + M4: LangGraph stack (no clustering / no Streamlit)
# pip install -e ".[ui]"          # Streamlit only (usually combined with other extras)
...
**Interactive mode** — chat with the agent through Streamlit:
