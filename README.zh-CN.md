...
**ReAct 编排器 + Streamlit UI** — 自然语言界面将整个流水线封装为 LangChain 工具（`attribute_domain`、`process_domains`、`search_cti`）。Streamlit 对话界面附带置信度面板与元数据展示。
...
│   │   ├── routing.py       #     确定性模板选择
│   │   ├── orchestrator.py  #     ReAct 智能体 + 工具定义
│   │   ├── nodes/
...
# pip install -e ".[clustering]" # 较轻：核心 + M3 聚类/战役相关依赖（不装 LangGraph/Streamlit）
